#!/usr/bin/env python3
"""Plugin packaging lockstep + emit smoke."""
...
class PluginPackagingTests(unittest.TestCase):
    ...
    def test_emit_all_langs_and_modes(self) -> None:
        from emit import emit
        from ir import load_sample

        with tempfile.TemporaryDirectory() as td:
            out = Path(td)
            written = emit(load_sample(), out)
            ...
            expected = [
                ...
                "typescript/graph/workflow.ts",
                "go/agent/agent.go",
                "go/graph/workflow.go",
                "java/agent/src/main/java/com/spillwave/ager/adk/AgentApp.java",
                "java/graph/src/main/java/com/spillwave/ager/adk/WorkflowApp.java",
                "kotlin/agent/src/main/kotlin/com/spillwave/ager/adk/AgentApp.kt",
                "kotlin/graph/src/main/kotlin/com/spillwave/ager/adk/WorkflowApp.kt",
            ]
            for rel in expected:
    ...
    def test_emit_lang_filter(self) -> None:
        from emit import emit
        from ir import load_sample

        with tempfile.TemporaryDirectory() as td:
            out = Path(td)
            emit(load_sample(), out, lang="java", mode="graph")
            self.assertTrue((out / "java/graph/src/main/java/com/spillwave/ager/adk/WorkflowApp.java").is_file())
            self.assertFalse((out / "python/agent/agent.py").is_file())
